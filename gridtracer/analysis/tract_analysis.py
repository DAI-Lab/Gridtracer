...
def generate_wkb_hex(geometry: BaseGeometry, source_crs: str, target_crs: str = "EPSG:5070") -> str:
...
def analyze_all_tracts(geojson_path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        for tract_code in sorted(unique_tracts):
            ...
            total_housing = tract_blocks["HOUSING20"].sum() if "HOUSING20" in tract_blocks.columns else 0
        ...
        raise FileNotFoundError(f"File not found: {geojson_path}") from e
    except Exception as e:
        raise RuntimeError("Error processing data") from e


def aggregate_tract_data(geojson_path: str, tract_code: str, block_code: Optional[str] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        total_housing = target_data["HOUSING20"].sum() if "HOUSING20" in target_data.columns else 0
...
""")
    ...
    for tract in result["tract_summaries"]:
        ...
        print(
            f"Tract {tract['tract_code']:>6} | Pop: {tract['total_population']:>6,} | "
...
def create_interactive_map(result: Dict[str, Any], output_path: str) -> None:
    ...
    if not result["block_code"]:
        ...
        for _, block in blocks_gdf.iterrows():
            ...
            """
            block_gdf = gpd.GeoDataFrame([block.geometry], columns=["geometry"], crs=result["crs"])
            folium.GeoJson(
                block_gdf.to_json(),
                style_function=lambda x: {
                    "fillColor": "blue",
...
def create_all_tracts_static_map(result: Dict[str, Any], output_path: str) -> None:
    ...
    for _, (tract, color) in enumerate(zip(result["tract_summaries"], colors)):
        tract_gdf = gpd.GeoDataFrame([1], geometry=[tract["union_geometry"]], crs=result["crs"])
    ...
    ax.text(
        ...
        color="black",
        fontweight="bold",
        bbox={"facecolor": "white", "alpha": 0.8, "edgecolor": "black", "boxstyle": "round,pad=0.5"},
    )
...
def create_static_map(result: Dict[str, Any], output_path: str) -> None:
    ...
    ax.text(
        ...
        color="black",
        fontweight="bold",
        bbox={"facecolor": "white", "alpha": 0.7, "edgecolor": "none", "boxstyle": "round,pad=0.5"},
    )
...
def main() -> None:
    ...
    parser.add_argument("--output-html", help="Output path for the interactive HTML map (optional)")
    ...
    try:
        if args.tract_code:
            ...
            result = aggregate_tract_data(args.geojson_path, args.tract_code, args.block_code)
