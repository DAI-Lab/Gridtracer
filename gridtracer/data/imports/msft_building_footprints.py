...
class MicrosoftBuildingsDataHandler(DataHandler):
    ...
    def __init__(self, orchestrator: "WorkflowOrchestrator"):
        ...
        self.mapping_file = self.dataset_output_dir / "us_state_quadkey_mapping.json"
    ...
    def _tile_xy_to_lat_lon(self, tile_x: int, tile_y: int, zoom_level: int) -> Tuple[float, float, float, float]:
        """
        Convert tile X, Y coordinates to latitude/longitude bounding box.
        """
        import math

        def pixel_to_lat_lon(pixel_x: float, pixel_y: float, zoom_level: int) -> Tuple[float, float]:
    ...
    def _create_state_quadkey_mapping(self) -> Dict:
        ...
        for _, row in us_links.iterrows():
            ...
            try:
                ...
                if not (
                    -180 <= min_lon <= 180 and -90 <= min_lat <= 90 and -180 <= max_lon <= 180 and -90 <= max_lat <= 90
                ):
                    continue
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.logger.info(f"Downloading US states to temporary file: {temp_file}")
    ...
    def _filter_quadkeys_by_region(self, state_abbr: str) -> List[str]:
        ...
        # Perform spatial intersection
        intersecting_quadkeys = gpd.sjoin(quadkeys_gdf, region_boundary, how="inner", predicate="intersects")
    ...
    def _download_state_buildings(self, state_abbr: str, max_tiles: Optional[int] = None) -> List[Path]:
        """
        ...
        if state_abbr not in self.state_mapping:
            ...
            raise ValueError(f"State '{state_abbr}' not found in mapping. Available: {available_states}")
