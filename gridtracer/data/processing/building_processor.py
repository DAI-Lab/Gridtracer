...
class BuildingProcessor:
    ...
    def process(
        ...
        census_data: Dict,
        osm_data: Dict,
        microsoft_buildings_data: Dict,
        nrel_vintage_distribution: Dict,
    ):
        """
        ...
        all_buildings = self.calculate_floors(all_buildings, microsoft_buildings_data.get("ms_buildings"))

        # Step 4: Assign building IDs
        self.logger.info("Assigning building IDs")
        all_buildings = self._assign_building_id(all_buildings, census_data.get("target_region_blocks"))
        ...
        other.to_file(self.dataset_output_dir / "07_Final_other_buildings_with_building_id.geojson")

        if len(residential) > 0:
            self.logger.info(f"Step 6: Processing {len(residential)} residential buildings to determine building type")

            # Building type classification
            residential = self.classify_building_type(residential, census_data.get("target_region_blocks"))

            # Allot occupants based on census
            residential = self._allot_occupants(residential, census_data.get("target_region_blocks"))

            # Allot construction year
            self.logger.info("Step 7: Alloting construction year")
            residential = self._allot_construction_year(residential, nrel_vintage_distribution)
            residential.to_file(
                self.dataset_output_dir / "07_Final_residential_buildings_with_building_id_after_allotment.geojson"
            )
        ...
        if len(other) > 0:
            other = other.rename(columns={"id": "osm_id", "floor_area": "area", "building_use": "use"})
    ...
    def _filter_small_buildings(self, buildings: gpd.GeoDataFrame, min_area: int = 45) -> gpd.GeoDataFrame:
        """
        ...
        buildings = buildings[buildings["floor_area"] >= min_area]
        self.logger.info(
    ...
    def _calculate_floor_area(self, buildings: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
        """
    ...
    def classify_building_use(
        ...
        self.logger.info(f"Starting building use classification for {len(classified_buildings)} buildings.")
        ...
        if "building" in classified_buildings.columns:
            ...
            self.logger.info(f"Removed {excluded_count} non-habitable structures based on 'building' tag.")
        ...
        if "building" in candidate_buildings.columns:
            ...
            res_types = [
                ...
                "cabin",
                "apartments",
            ]  # hut/cabin often residential
            mask = candidate_buildings["building"].isin(res_types)
            candidate_buildings.loc[mask, "building_use"] = "residential"
            self.logger.debug(f"{mask.sum()} buildings classified as residential from 'building' tag.")
            ...
            mask = candidate_buildings["building"].isin(com_types) & candidate_buildings["building_use"].isna()
            candidate_buildings.loc[mask, "building_use"] = "commercial"
            self.logger.debug(f"{mask.sum()} buildings classified as commercial from 'building' tag.")
            ...
            mask = candidate_buildings["building"].isin(ind_types) & candidate_buildings["building_use"].isna()
            candidate_buildings.loc[mask, "building_use"] = "industrial"
            self.logger.debug(f"{mask.sum()} buildings classified as industrial from 'building' tag.")
            ...
            mask = candidate_buildings["building"].isin(pub_types) & candidate_buildings["building_use"].isna()
            candidate_buildings.loc[mask, "building_use"] = "public"
            self.logger.debug(f"{mask.sum()} buildings classified as public from 'building' tag.")

        # 1.2 'building:use' tag (often more specific than 'building')
        if "building:use" in candidate_buildings.columns:
            use_map = {
                "residential": "residential",
                "residental": "residential",  # common typo
            ...
            self.logger.debug("Applied 'building:use' tag classifications. Check specific counts if needed.")
        ...
        if "amenity" in candidate_buildings.columns:
            ...
            mask = (
                candidate_buildings["amenity"].fillna("").str.lower().isin(pub_amenities)
                & candidate_buildings["building_use"].isna()
            )
            candidate_buildings.loc[mask, "building_use"] = "public"
            self.logger.debug(f"{mask.sum()} buildings classified as public from 'amenity' tag.")

            mask = (
                candidate_buildings["amenity"].fillna("").str.lower().isin(com_amenities)
                & candidate_buildings["building_use"].isna()
            )
            candidate_buildings.loc[mask, "building_use"] = "commercial"
            self.logger.debug(f"{mask.sum()} buildings classified as commercial from 'amenity' tag.")

            mask = (
                candidate_buildings["amenity"].fillna("").str.lower().isin(res_amenities)
                & candidate_buildings["building_use"].isna()
            )
            candidate_buildings.loc[mask, "building_use"] = "residential"
            self.logger.debug(f"{mask.sum()} buildings classified as residential from 'amenity' (shelter).")

        # 1.4 'shop' tag
        if "shop" in candidate_buildings.columns:
            mask = candidate_buildings["shop"].notna() & candidate_buildings["building_use"].isna()
            candidate_buildings.loc[mask, "building_use"] = "commercial"
            self.logger.debug(f"{mask.sum()} buildings classified as commercial from 'shop' tag.")
        ...
        if "office" in candidate_buildings.columns:
            ...
            candidate_buildings.loc[mask, "building_use"] = "public"
            self.logger.debug(f"{mask.sum()} buildings classified as Public from specific 'office' types.")

            # General commercial offices
            mask = (
                candidate_buildings["office"].notna()
                & ~candidate_buildings["office"].fillna("").str.lower().isin(public_office_types)
                & candidate_buildings["building_use"].isna()
            )
            candidate_buildings.loc[mask, "building_use"] = "commercial"
            self.logger.debug(f"{mask.sum()} buildings classified as commercial from general 'office' tag.")

        # 1.6 'building:flats' tag
        if "building:flats" in candidate_buildings.columns:
            mask = candidate_buildings["building:flats"].notna() & candidate_buildings["building_use"].isna()
            candidate_buildings.loc[mask, "building_use"] = "residential"
            self.logger.debug(f"{mask.sum()} buildings classified as Residential from 'building:flats' tag.")

        # 1.7 'craft' tag
        if "craft" in candidate_buildings.columns:
            mask = candidate_buildings["craft"].notna() & candidate_buildings["building_use"].isna()
            # Or 'Industrial' for some
            candidate_buildings.loc[mask, "building_use"] = "commercial"
            self.logger.debug(f"{mask.sum()} buildings classified as commercial from 'craft' tag.")
        ...
        self.logger.debug("Starting POI-based classification for remaining unclassified buildings.")

        # Ensure POIs GeoDataFrame is provided and not empty
        if pois is None or pois.empty:
            self.logger.warning("POIs GeoDataFrame is missing or empty. Skipping POI-based classification.")
        else:
            ...
            buildings_to_classify_via_poi = classified_buildings[classified_buildings["building_use"].isna()].copy()

            if not buildings_to_classify_via_poi.empty:
                try:
                    buildings_with_pois = gpd.sjoin(
                        buildings_to_classify_via_poi,
                        pois,
                        how="left",
                ...
                if poi_index_col_name not in buildings_with_pois.columns:
                    ...
                    if "index_poi" not in buildings_with_pois.columns:
                        self.logger.error(
                            "Critical: Fallback POI index column 'index_poi' also not found. Cannot filter POI matches."
                        )
                ...
                if not buildings_with_pois.empty and poi_index_col_name in buildings_with_pois.columns:
                    buildings_with_pois_matches = buildings_with_pois[
                        buildings_with_pois[poi_index_col_name].notna()
                    ].copy()
                    self.logger.debug(f"{len(buildings_with_pois_matches)} building-POI intersections found.")
                else:
                    # Ensure it's an empty GeoDataFrame
                    buildings_with_pois_matches = gpd.GeoDataFrame()
                    self.logger.debug("No building-POI intersections found or POI index column missing.")
                ...
                for building_idx in unique_building_indices_with_poi_match:
                    ...
                    if pd.isna(classified_buildings.loc[building_idx, "building_use"]):
                        ...
                        if "amenity_poi" in pois_for_this_building.columns:
                            if pois_for_this_building["amenity_poi"].isin(poi_public_amenities).any():
                        ...
                        if not classified_this_building and "amenity_poi" in pois_for_this_building.columns:
                            if pois_for_this_building["amenity_poi"].isin(poi_commercial_amenities).any():
                                classified_buildings.loc[building_idx, "building_use"] = "commercial"
                                classified_this_building = True

                        # Rule 3: POI Amenity for Residential (if not already
                        # classified)
                        if not classified_this_building and "amenity_poi" in pois_for_this_building.columns:
                            if pois_for_this_building["amenity_poi"].isin(poi_residential_amenities).any():
                                classified_buildings.loc[building_idx, "building_use"] = "residential"
                                classified_this_building = True
                        ...
                        if not classified_this_building and "office_poi" in pois_for_this_building.columns:
                            ...
                            if pois_for_this_building["office_poi"].isin(poi_public_office_types).any():
                                classified_buildings.loc[building_idx, "building_use"] = "public"
                                classified_this_building = True
                            # Any other office
        ...
        self.logger.info("Starting Landuse-based classification for remaining unclassified buildings.")
        if landuse is None or landuse.empty:
            self.logger.warning("Landuse GeoDataFrame is missing or empty. Skipping Landuse-based classification.")
        else:
            ...
            buildings_to_classify_via_landuse = classified_buildings[classified_buildings["building_use"].isna()].copy()

            if not buildings_to_classify_via_landuse.empty:
                try:
                    buildings_with_landuse = gpd.sjoin(
                        buildings_to_classify_via_landuse,
                        landuse,
                        how="left",
                        predicate="within",
                        lsuffix="",
                ...
                if landuse_join_index_col_name not in buildings_with_landuse.columns:
                    ...
                    if "index_landuse" not in buildings_with_landuse.columns:
                        self.logger.error(
                            "Critical: Fallback Landuse index column 'index_landuse' also not found. Cannot filter landuse matches."
                        )
                ...
                if not buildings_with_landuse.empty and landuse_join_index_col_name in buildings_with_landuse.columns:
                    buildings_with_landuse_matches = buildings_with_landuse[
                        buildings_with_landuse[landuse_join_index_col_name].notna()
                    ].copy()
                else:
                ...
                if landuse_col_name not in buildings_with_landuse_matches.columns and "landuse" in landuse.columns:
                    ...
                    potential_cols = [
                        col
                        for col in buildings_with_landuse_matches.columns
                        if col.startswith("landuse") and col.endswith("_landuse")
                    ]
                    if potential_cols:
                ...
                if landuse_col_name:
                    ...
                    for building_idx in unique_building_indices_with_landuse_match:
                        if pd.isna(classified_buildings.loc[building_idx, "building_use"]):
                            landuses_for_building = buildings_with_landuse_matches.loc[[building_idx]]
                            ...
                            if pd.notna(current_use):
                                classified_buildings.loc[building_idx, "building_use"] = current_use
        ...
        self.logger.debug(f"{mask.sum()} buildings assigned default use 'residential'.")

        # Sanity check: Residential buildings cannot be bigger than 800 sq
        # meters, then set to Commercial
        classified_buildings.loc[classified_buildings["floor_area"] > 1000, "building_use"] = "commercial"
    ...
    def _cleaning_osm_data(self, gdf: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
        ...
        if "tags" in gdf.columns:
            ...
            gdf["tags"] = gdf["tags"].apply(
                lambda d: {k: v for k, v in d.items() if k not in tag_keys_to_drop} if isinstance(d, dict) else {}
            )
    ...
    def calculate_free_walls(self, buildings: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
        ...
        buildings_with_walls["neighbors"] = buildings_with_walls.index.map(lambda idx: neighbors_dict.get(idx, []))
    ...
    def classify_building_type(
        self, buildings: gpd.GeoDataFrame, housing_data: Optional[Dict] = None
    ) -> gpd.GeoDataFrame:
        """
        ...
        classified_buildings["neighbors"] = classified_buildings.index.map(lambda idx: neighbors_dict.get(idx, []))
        ...
        classified_buildings["cluster"] = classified_buildings.index.map(
            lambda idx: sorted(clusters_dict.get(idx, {idx}))
        )
        ...
        self.logger.debug("Classifying building types based on cluster characteristics")
    ...
    def _assign_building_types(self, buildings: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
        ...
        for cluster in ab_clusters:
            cluster_mask = buildings["cluster"].apply(lambda x, c=cluster: x == c)
        ...
        sfh_mask = (
            (buildings["floor_area"] <= BUILDING_TYPE_THRESHOLDS["SFH_MAX_FLOOR_AREA"])
        ...
        if "height" in buildings.columns:
            height_constraint = (buildings["height"].isna()) | (
                pd.to_numeric(buildings["height"], errors="coerce") < BUILDING_TYPE_THRESHOLDS["SFH_MAX_HEIGHT"]
            )
        ...
        th_mask = (
            (buildings["neighbors"].apply(len) >= BUILDING_TYPE_THRESHOLDS["TH_MIN_NEIGHBORS"])
        ...
        if "height" in buildings.columns:
            height_constraint = buildings["height"].isna() | (
                pd.to_numeric(buildings["height"], errors="coerce") <= BUILDING_TYPE_THRESHOLDS["TH_MAX_HEIGHT"]
            )
        ...
        th_clusters = buildings[buildings["building_type"] == "TH"]["cluster"].tolist()
        for cluster in th_clusters:
            # Unclassified buildings in this cluster that meet TH criteria
            cluster_mask = (
                buildings["cluster"].apply(lambda x, c=cluster: x == c)
                & buildings["building_type"].isna()
                & (buildings["floor_area"] <= BUILDING_TYPE_THRESHOLDS["TH_MAX_FLOOR_AREA"])
            )
    ...
    def _allot_occupants(self, buildings: gpd.GeoDataFrame, census_blocks: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
        """
        ...
        for _, census_block in census_blocks.iterrows():
            ...
            block_buildings["initial_capacity"] = block_buildings.apply(calculate_initial_capacity, axis=1)
            ...
            self.logger.debug(
                f"Block {geoid}: {remaining_population} people, "
            ...
            if remaining_population > total_initial_capacity and ab_count == 0:
                self.logger.debug(f"Block {geoid}: Population exceeds capacity, adjusting building capacities")

                # Increase MFH capacity first
                if mfh_count > 0:
                    while total_initial_capacity < remaining_population:
                        for idx in block_buildings[block_buildings["building_type"] == "MFH"].index:
                ...
                if sfh_count > 0 and total_initial_capacity < remaining_population:
                    while total_initial_capacity < remaining_population:
                        for idx in block_buildings[block_buildings["building_type"] == "SFH"].index:
                            block_buildings.at[idx, "max_capacity"] += 1
                        total_initial_capacity = block_buildings["max_capacity"].sum()
                        if total_initial_capacity >= remaining_population:
                            break
            ...
            self.logger.debug(f"Block {geoid}: Starting incremental allocation of {remaining_population} people")
            ...
            while remaining_population > 0 and iteration_count < max_iterations:
                ...
                if not allocated_this_round:
                    self.logger.warning(f"Block {geoid}: Could not allocate remaining {remaining_population} people")
                    break
            ...
            if total_allocated_population > 0 and total_units > 0:
                for idx in block_buildings.index:
                    occupant_ratio = block_buildings.at[idx, "occupants"] / total_allocated_population
                    block_buildings.at[idx, "housing_units"] = int(round(occupant_ratio * total_units))
            else:
                # Fallback: estimate housing units from occupants
                block_buildings["housing_units"] = (block_buildings["occupants"] / 2.6).round().astype(int)

            # Update the master dataframe
            buildings_with_occupants.loc[mask, "occupants"] = block_buildings["occupants"]
            buildings_with_occupants.loc[mask, "housing_units"] = block_buildings["housing_units"].astype(int)

            # Log allocation results
            final_allocated = block_buildings["occupants"].sum()
            self.logger.debug(
                f"Block {geoid}: Allocated {final_allocated}/{total_pop} people "
        ...
        if len(zero_occupant_buildings) > 0:
            for idx in zero_occupant_buildings.index:
                ...
                if building_type in building_stats:
                    estimated_occupants = max(1, int(building_stats[building_type]["max_statistical"] / 4))
                    buildings_with_occupants.at[idx, "occupants"] = estimated_occupants
                    # Estimate housing units and cast to int
                    buildings_with_occupants.at[idx, "housing_units"] = max(1, int(round(estimated_occupants / 2.6)))
        ...
        for idx in buildings_with_occupants.index:
            ...
            if building_type in building_stats:
                ...
                if current_occupants > max_allowed:
                    ...
                    buildings_with_occupants.at[idx, "housing_units"] = max(1, int(round(max_allowed / 2.6)))

        return buildings_with_occupants

    def _assign_building_id(self, buildings: gpd.GeoDataFrame, census_blocks: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
        """
        ...
        if missing_cols:
            self.logger.error(f"Census blocks missing required columns: {missing_cols}. Cannot assign IDs.")
            buildings_copy = buildings.copy()
            buildings_copy["building_id"] = None
            buildings_copy["census_block_id"] = None
            if not buildings_copy.empty:
                ...
                buildings_copy.to_file(
                    self.dataset_output_dir / "5_buildings_unassigned_to_blocks.geojson",
                    driver="GeoJSON",
                )
            return buildings_copy
        ...
        try:
            ...
            self.logger.error(f"Error during CRS projection to {target_crs}: {e}. Cannot assign IDs.")
            buildings_copy = buildings.copy()
            buildings_copy["building_id"] = None
            buildings_copy["census_block_id"] = None
            if not buildings_copy.empty:
                self.dataset_output_dir.mkdir(parents=True, exist_ok=True)
                buildings_copy.to_file(
                    self.dataset_output_dir / "unassigned_buildings.geojson",
                    driver="GeoJSON",
                )
            return buildings_copy
        ...
        if len(assigned_buildings) > 0:
            ...
            assignments_df["building_id"] = assignments_df["GEOID20"] + assignments_df["sequential_id"].astype(
                str
            ).str.zfill(4)

            # Vectorized assignment back to original buildings
            buildings_with_ids.loc[assignments_df["original_building_idx"], "building_id"] = assignments_df[
                "building_id"
            ].values
            buildings_with_ids.loc[assignments_df["original_building_idx"], "census_block_id"] = assignments_df[
                "GEOID20"
            ].values
        ...
        if unassigned_count > 0:
            self.logger.warning(
                f"Found {unassigned_count} buildings whose centroids fall outside "
        ...
        if len(buildings) > 0:
            ...
            self.logger.info(
                f"Building ID assignment complete: {total_assigned}/{len(buildings)} "
    ...
    def _allot_construction_year(
        self, buildings: gpd.GeoDataFrame, nrel_vintage_distribution: Dict[str, float]
    ) -> gpd.GeoDataFrame:
        """
        ...
        assigned_vintages = np.random.choice(vintage_bins, size=len(buildings), p=probabilities)

        buildings = buildings.copy()
        buildings["construction_year"] = assigned_vintages

        self.logger.info(f"Assigned construction years to {len(buildings)} buildings. ")
    ...
    def calculate_floors(self, buildings: gpd.GeoDataFrame, microsoft_buildings: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
        """
        ...
        self.logger.info(f"Starting floors/height calculation for {initial_total_buildings} buildings")
        ...
        undetermined_buildings = buildings_with_floors[
            buildings_with_floors["height"].isna() | buildings_with_floors["floors"].isna()
        ]

        if len(undetermined_buildings) > 0:
            processed_undetermined = self._calculate_floor_height_from_osm_tags(undetermined_buildings)
        ...
        height_pct = (height_count / total_buildings * 100) if total_buildings > 0 else 0.0
        floors_pct = (floors_count / total_buildings * 100) if total_buildings > 0 else 0.0
        self.logger.info(f"Completed floors/height calculation: height={height_pct:.1f}%, floors={floors_pct:.1f}%")

        return buildings_with_floors

    def _calculate_floor_height_from_osm_tags(self, buildings: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
        """
        ...
        if "height" in buildings_with_height_floors.columns:
            ...
            if height_mask.any():
                buildings_with_height_floors.loc[height_mask, "height"] = buildings_with_height_floors.loc[
                    height_mask, "height"
                ].apply(self._parse_height_value)
        ...
        if "building:levels" in buildings_with_height_floors.columns:
            ...
            if levels_mask.any():
                buildings_with_height_floors.loc[levels_mask, "floors"] = buildings_with_height_floors.loc[
                    levels_mask, "building:levels"
                ].apply(self._parse_floor_value)
        ...
        height_no_floors_mask = (buildings_with_height_floors["height"].notna()) & (
            buildings_with_height_floors["floors"].isna()
        )  # Fixed mask logic

        if height_no_floors_mask.any():
            # Estimate floors using helper method
            buildings_with_height_floors.loc[height_no_floors_mask, "floors"] = buildings_with_height_floors.loc[
                height_no_floors_mask, "height"
            ].apply(self._height_to_floors)
            self.logger.info(f"Estimated floors from height for {height_no_floors_mask.sum()} buildings")

        # Handle cases where we have floors but no height - estimate height from
        floors_no_height_mask = (buildings_with_height_floors["height"].isna()) & (
            buildings_with_height_floors["floors"].notna()
        )  # Fixed mask logic

        if floors_no_height_mask.any():
            # Estimate height using helper method
            buildings_with_height_floors.loc[floors_no_height_mask, "height"] = buildings_with_height_floors.loc[
                floors_no_height_mask, "floors"
            ].apply(self._floors_to_height)
            self.logger.info(f"Estimated height from floors for {floors_no_height_mask.sum()} buildings")
        ...
        if "building:min_level" in buildings_with_height_floors.columns:
            ...
            if min_level_mask.any():
                self.logger.debug(f"Found {min_level_mask.sum()} buildings with minimum level information")
        ...
        self.logger.debug("OSM tag processing complete:")
        self.logger.debug(
    ...
    def _calculate_floor_height_from_ms_buildings(
        ...
        if "properties" in ms_buildings_copy.columns and "height" not in ms_buildings_copy.columns:
            self.logger.debug("Height data appears to be nested in properties column, extracting...")
            ...
            ms_buildings_copy["confidence"] = ms_buildings_copy["properties"].apply(extract_confidence)
        ...
        height_mask = (ms_buildings_projected["height"].notna()) & (
            pd.to_numeric(ms_buildings_projected["height"], errors="coerce") > 0.5
        )

        valid_ms_buildings = ms_buildings_projected[height_mask].copy()

        if len(valid_ms_buildings) == 0:
            self.logger.debug("No valid height data found in Microsoft Buildings (all heights are -1 or invalid)")
        ...
        self.logger.debug("Calculating centroids of Microsoft Buildings for robust spatial matching")
        ...
        if "confidence" in ms_centroids_gdf.columns:
            ...
            high_confidence_mask = (
                pd.to_numeric(ms_centroids_gdf["confidence"], errors="coerce") >= confidence_threshold
            )
        ...
        try:
            ...
            if "confidence_ms" in joined.columns:
                agg_dict["confidence_ms"] = ["mean"]  # <- list, not str
    ...
    def write_buildings_output(
        ...
        output_dir = Path(output_dir) / "SHP"
