...
def plot_subdivisions(csv_path: Path, show_labels: bool = True):
    ...
    gdf_web_mercator.plot(ax=ax, edgecolor="blue", facecolor="yellow", alpha=0.5, linewidth=1.5)
    ...
    if show_labels:
        ...
        for _, row in gdf_web_mercator.iterrows():
            ...
            ax.text(
                ...
                va="center",
                fontsize=8,
                bbox={"boxstyle": "round,pad=0.3", "fc": "white", "ec": "black", "alpha": 0.7},
            )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Plot state subdivisions from a processed CSV file.")
    parser.add_argument(
        "csv_path",
        type=str,
