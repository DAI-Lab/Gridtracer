...
def handler(mock_orchestrator: Mock) -> MicrosoftBuildingsDataHandler:
    ...
    with patch(
        "gridtracer.data.imports.msft_building_footprints.MSFT_BUILD_FOOTPRINTS",
        {
...
class TestMicrosoftBuildingsDataHandler:
    ...
    def test_tile_xy_to_lat_lon(self, handler: MicrosoftBuildingsDataHandler):
        ...
        min_lat, min_lon, max_lat, max_lon = handler._tile_xy_to_lat_lon(tile_x, tile_y, zoom_level)
    ...
    def test_load_state_mapping_existing_file(self, handler: MicrosoftBuildingsDataHandler, sample_state_mapping: Dict):
    ...
    def test_load_state_mapping_create_new(self, handler: MicrosoftBuildingsDataHandler, sample_state_mapping: Dict):
        """Test creating new state mapping when file doesn't exist."""
        with patch("pathlib.Path.exists", return_value=False):
            with patch.object(
                handler,
                "_create_state_quadkey_mapping",
                return_value=sample_state_mapping,
            ):
                result = handler._load_state_mapping()

                assert result == sample_state_mapping

    def test_filter_quadkeys_by_region(self, handler: MicrosoftBuildingsDataHandler, sample_state_mapping: Dict):
        """Test filtering quadkeys by region boundary."""
        handler.state_mapping = sample_state_mapping

        with patch("geopandas.sjoin") as mock_sjoin:
            # Mock the spatial join result
            mock_result = gpd.GeoDataFrame({"quadkey": ["1234567890", "1234567891"]})
    ...
    def test_filter_quadkeys_by_region_invalid_state(
        self, handler: MicrosoftBuildingsDataHandler, sample_state_mapping: Dict
    ):
    ...
    def test_download_state_buildings(self, handler: MicrosoftBuildingsDataHandler, sample_state_mapping: Dict):
    ...
    def test_filter_buildings_to_region_empty_files(self, handler: MicrosoftBuildingsDataHandler):
    ...
    def test_filter_buildings_to_region_invalid_files(self, handler: MicrosoftBuildingsDataHandler):
    ...
    def test_download_no_files(self, handler: MicrosoftBuildingsDataHandler):
    ...
    def test_download_success(
        ...
        with patch("pathlib.Path.exists", return_value=False):
            with patch.object(
                handler,
                "_download_state_buildings",
                return_value=[Path("/tmp/test.geojson")],
            ):
                with patch.object(
                    handler,
                    "_filter_buildings_to_region",
    ...
    def test_download_error_handling(self, handler: MicrosoftBuildingsDataHandler):
    ...
    def test_process_error_handling(self, handler: MicrosoftBuildingsDataHandler):
