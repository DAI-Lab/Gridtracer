...
class TestNRELDataHandlerInitialization:
    ...
    def test_initialization_missing_nrel_path(self, mock_workflow_config, sample_fips_csv_content, temp_output_dir):
        """Test initialization when NREL path is missing from config."""

        with patch(
            "gridtracer.data.imports.nrel.INPUT_DATA",
            {"OSM_PBF_FILE": "/path/to/osm.pbf"},
        ):
...
class TestChunkedProcessing:
    ...
    def test_extract_county_chunk_missing_column(self, orchestrator_with_fips):
        ...
        data_no_county = pd.DataFrame({"bldg_id": [1, 2, 3], "in.vintage": ["1970s", "1980s", "<1940"]})
...
class TestVintageDistribution:
    ...
    def test_compute_vintage_distribution_missing_column(self, orchestrator_with_fips):
        ...
        test_data = pd.DataFrame({"other_col": ["A", "B", "C"]})
