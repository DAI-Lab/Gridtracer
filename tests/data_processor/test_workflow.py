...
class TestWorkflowOrchestratorInitialization:
    ...
    def test_initialization_success(self, orchestrator_with_fips: WorkflowOrchestrator):
    ...
    def test_initialization_county_only(self, mock_workflow_config, sample_fips_csv_content, temp_output_dir):
        ...
        with patch("gridtracer.data.workflow.REGION", region_without_subdivision), patch(
    ...
    def test_initialization_missing_required_config(self, mock_workflow_config, temp_output_dir):
    ...
    def test_fips_resolution_invalid_state(self, mock_workflow_config, temp_output_dir):
        ...
        with patch("gridtracer.data.workflow.REGION", invalid_region), patch(
            "urllib.request.urlretrieve"
        ) as mock_urlretrieve:
            mock_urlretrieve.side_effect = lambda url, filepath: create_mock_fips_file(Path(filepath), fips_content)

            with pytest.raises(ValueError, match="Failed to lookup FIPS codes"):
                WorkflowOrchestrator()

    def test_fips_resolution_invalid_county(self, mock_workflow_config, temp_output_dir):
    ...
    def test_fips_file_download_failure(self, mock_workflow_config):
    ...
    def test_fips_csv_parsing_inconsistent_rows(self, mock_workflow_config, temp_output_dir):
        ...
        with patch("urllib.request.urlretrieve") as mock_urlretrieve:
            mock_urlretrieve.side_effect = lambda url, filepath: create_mock_fips_file(Path(filepath), inconsistent_csv)
...
class TestDirectoryManagement:
    ...
    def test_dataset_directory_creation(self, orchestrator_with_fips: WorkflowOrchestrator):
    ...
    def test_invalid_dataset_name(self, orchestrator_with_fips: WorkflowOrchestrator):
    ...
    def test_directory_structure_county_level(self, mock_workflow_config, sample_fips_csv_content, temp_output_dir):
        ...
        with patch("gridtracer.data.workflow.REGION", region_county_only), patch(
            "urllib.request.urlretrieve"
        ) as mock_urlretrieve:
            mock_urlretrieve.side_effect = lambda url, filepath: create_mock_fips_file(
                Path(filepath), sample_fips_csv_content
...
class TestRegionBoundaryManagement:
    ...
    def test_boundary_not_set_initially(self, orchestrator_with_fips: WorkflowOrchestrator):
    ...
    def test_boundary_overwrite(
        ...
        new_boundary_data = {
            ...
            "geometry": [Polygon([(-72.0, 43.0), (-72.0, 43.1), (-71.9, 43.1), (-71.9, 43.0)])],
        }
...
class TestOSMParserIntegration:
    ...
    def test_osm_parser_without_boundary(self, orchestrator_with_fips: WorkflowOrchestrator, mock_osm_parser):
    ...
    def test_osm_parser_lazy_loading(self, orchestrator_with_fips: WorkflowOrchestrator, mock_osm_parser):
    ...
    def test_osm_parser_initialization_error(
        ...
        with patch("pathlib.Path.exists", return_value=True), patch(
            "gridtracer.data.workflow.OSM", side_effect=Exception("OSM init failed")
        ):
            parser = orchestrator.get_osm_parser()
            assert parser is None
...
class TestErrorHandling:
    ...
    def test_malformed_fips_file(self, mock_workflow_config, temp_output_dir):
        """Test handling of malformed FIPS file."""
        malformed_csv = "invalid,csv,content\n"

        with patch("urllib.request.urlretrieve") as mock_urlretrieve:
            mock_urlretrieve.side_effect = lambda url, filepath: create_mock_fips_file(Path(filepath), malformed_csv)

            with pytest.raises(ValueError, match="Failed to lookup FIPS codes"):
                WorkflowOrchestrator()

    def test_empty_fips_file(self, mock_workflow_config, temp_output_dir):
        """Test handling of empty FIPS file."""
        empty_csv = ""

        with patch("urllib.request.urlretrieve") as mock_urlretrieve:
            mock_urlretrieve.side_effect = lambda url, filepath: create_mock_fips_file(Path(filepath), empty_csv)
...
class TestIntegrationScenarios:
    ...
    def test_typical_data_handler_workflow(
        ...
        with patch("pathlib.Path.exists", return_value=True), patch("gridtracer.data.workflow.OSM") as mock_osm:
            mock_osm.return_value = Mock()
            parser = orchestrator.get_osm_parser()
            assert parser is not None

    def test_subdivision_vs_county_processing(self, mock_workflow_config, sample_fips_csv_content, temp_output_dir):
        ...
        with patch("gridtracer.data.workflow.REGION", region_with_subdivision), patch(
            "urllib.request.urlretrieve"
        ) as mock_urlretrieve:
            mock_urlretrieve.side_effect = lambda url, filepath: create_mock_fips_file(
                Path(filepath), sample_fips_csv_content
